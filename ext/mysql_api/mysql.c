...
static void mysql_raise(MYSQL* m, char *note)
{
    char *message = mysql_error(m);
    VALUE rb_message = rb_str_new2(message);
    VALUE e;
    if(strlen(message) > 0) {
        rb_str_cat2(rb_message, ", mysql_raise, ");
        rb_str_cat2(rb_message, note);
        e = rb_exc_new3(eMysql, rb_message);
    } else {
        rb_str_cat2(rb_message, "no error message, mysql_raise, ");
        rb_str_cat2(rb_message, note);
        e = rb_exc_new3(eMysql, rb_message);
    }
